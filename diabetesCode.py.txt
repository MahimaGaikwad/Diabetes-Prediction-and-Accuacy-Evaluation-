...
trainingData = read_csv('../input/diabetes.csv') 
trainingData=trainingData[['Pregnancies','Glucose','BloodPressure','SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age','Outcome']]
testingData = read_csv('../input/diabetes.csv')
testingData=testingData[['Pregnancies','Glucose','BloodPressure','SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age']]
...
trainingFeatures.columns=['Pregnancies','Glucose','BloodPressure','SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age']
testingData = imputer.transform(testingData)
testingData = pd.DataFrame(testingData)
testingData.columns=['Pregnancies','Glucose','BloodPressure','SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age']
...
trainingFeatures2 = trainingFeatures.drop(['Pregnancies','BloodPressure','DiabetesPedigreeFunction', 'Age','SkinThickness','Insulin'], axis=1)

g = sns.heatmap(trainingFeatures2.corr(),cmap="Blues",annot=False)
print("CORRELATION BETWEEN TRAINING FEATURES ")
...
test2 = test.drop(['Pregnancies','BloodPressure','DiabetesPedigreeFunction', 'Age','SkinThickness','Insulin'], axis=1)
my_predictions = model.predict(test2)
my_submission = pd.DataFrame({'Outcome': my_predictions})
my_submission.to_csv('my_submission.csv', index=False)
my_submission.head()

output_csv=read_csv('my_submission.csv')
print(output_csv)
...
predictedDie = model.predict([[Glucose,BMI]])
df1=pd.DataFrame({'Predicted': predictedDie})
print(df1)
